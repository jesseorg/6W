...
private struct BrowserViewControllerUX {
	...
	fileprivate static let BackgroundColor = UIColor.green
...
class BrowserViewController: UIViewController {
    ...
    func SELappDidBecomeActiveNotification() {
        ...
        UIView.animate(withDuration: 0.1, delay: 0, options: UIViewAnimationOptions(), animations: {
			//self.webViewContainer.alpha = 0.3 //jesse this is when the app comes back from a notification
			//self.urlBar.locationContainer.alpha = 0.3//jesse
			self.topTabsViewController?.switchForegroundStatus(isInForeground: true)
			self.presentedViewController?.popoverPresentationController?.containerView?.alpha = 1//jesse
			self.presentedViewController?.view.alpha = 1//jesse
    ...
    override func viewDidLoad() {
		...
		webViewContainerBackdrop = UIView() //jesse 24
		webViewContainerBackdrop.backgroundColor = UIColor.green
		webViewContainerBackdrop.alpha = 0.5
		view.addSubview(webViewContainerBackdrop)
		
		
		
		
        log.debug("Jesse setting up AR…")
		let ARviewController = CameraViewController()

 
		
		ARviewController.NOviewDidLoad() // this makes previewView
		
		if let previewView = ARviewController.previewView {
			...
			previewView.alpha=1
			log.debug("Jesse Success setting up AR…")
		}

		
		
			ARviewController.ARview(false)
        ...
        urlBar = URLBarView()
		urlBar.alpha = 0.5 //0.0001 //jesse 201808141214
		...
		urlBar.removeLocationTextField() // jesse
...
*/
tab.desktopSite = true
//
log.warning("tab.desktopSite true")
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
		...
		webView.backgroundColor = UIColor.clear //jesse
		webView.isOpaque = false //jesse
    ...
    fileprivate func resetSpoofedUserAgentIfRequired(_ webView: WKWebView, newURL: URL) {
		...
		// jesse 20180730 // dont do this. leave it.
      /*
...
extension BrowserViewController: MenuActionDelegate {
    func performMenuAction(_ action: MenuAction, withAppState appState: AppState) {
        if let menuAction = AppMenuAction(rawValue: action.action) {
			...
			case .showWX: //jesse
			log.warning("jesse.showWX")
			
			
			/*
			
				guard let tab = tabManager.selectedTab else { break }
				guard let WXtab = tabManager.selectedTab else { break }
				WXtab.desktopSite = true;
				let requestDesktopSiteActivity = RequestDesktopSiteActivity(requestMobileSite: WXtab.desktopSite) { [unowned tab] in
                WXtab.toggleDesktopSite()
				}
			
			
				log.warning("jesse.requestDesktopSiteActivity")
				var activities = [UIActivity]()
				activities.append(requestDesktopSiteActivity)
			...
			*/
				...
				if let fixupURL = URIFixup.getURL("http://wx.qq.com") {
