...
/*
import AVFoundation // needed to open camera / jesse 2017-5-15
import UIKit
...
class AppDelegate: UIResponder, UIApplicationDelegate, UIViewControllerRestoration {
	    ...
	    internal var previewView: UIView? //jesse
    ...
    @discardableResult func application(_ application: UIApplication, willFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
		...
		log.debug("Configuring color BG - -")
		//log.debug(UIConstants.AppBackgroundColor)
		...
		if let previewView = self.previewView {
			...
			previewView.layer.addSublayer(NextLevel.shared.previewLayer)
			    self.window?.addSubview(previewView)
			
			previewView.alpha=0.3;
			log.debug("Jesse root AR window…")
		}
	
		
		   */
		//previewView = CameraViewController()
		//self.window?.makeKeyAndVisible()
    ...
    @discardableResult fileprivate func startApplication(_ application: UIApplication, withLaunchOptions launchOptions: [AnyHashable: Any]?) -> Bool {
		...
		// jesse 20180621 
		// this assigns a root window and is picked up by browser view controller
